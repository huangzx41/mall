...
public class AccountController extends BaseController {
	...
	public String uploadAdminHeadImage(@RequestParam MultipartFile file, HttpSession session) {
		...
		// 获取上传路径
		String filePath = session.getServletContext().getRealPath("/") + "res/images/item/adminProfilePicture/"
				+ fileName;
	...
	public String updateAdmin(HttpSession session, @RequestParam String admin_nickname/* 管理员昵称 */,
		...
		if (admin_password != null && !"".equals(admin_password) && admin_newPassword != null
				&& !"".equals(admin_newPassword)) {
			logger.info("获取需要修改的管理员信息");
			Admin admin = adminService.get(null, Integer.valueOf(adminId.toString()));
			if (adminService.login(admin.getAdmin_name()).getAdmin_password()
					.equals(SecureUtil.sha256(admin_password))) {
				logger.info("原密码正确");
		...
		if (admin_profile_picture_src != null && !"".equals(admin_profile_picture_src)) {
			...
			putAdmin.setAdmin_profile_picture_src(
					admin_profile_picture_src.substring(admin_profile_picture_src.lastIndexOf("/") + 1));
		}
